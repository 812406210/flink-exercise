            ...
            for (Map.Entry<String, String> entry : param.entrySet()){
                ...
                for (Field field : fields) {
                    ...
                    if(qualifier.equals(entry.getKey())){
                        Filter filter = new SingleColumnValueFilter(Bytes.toBytes(family), Bytes.toBytes(entry.getKey()), CompareFilter.CompareOp.EQUAL, Bytes
        ...
        try (
            ...
            for (Map.Entry<String, String> entry : param.entrySet()){
                ...
                for (Field field : fields) {
                    ...
                    String qualifier = orm.qualifier();
                    if(qualifier.equals(entry.getKey())){
                        Filter filter = new SingleColumnValueFilter(Bytes.toBytes(family), Bytes.toBytes(entry.getKey()), CompareFilter.CompareOp.GREATER_OR_EQUAL, Bytes
                                .toBytes(entry.getValue()));
                        scan.setFilter(filter);
                    }
                }
