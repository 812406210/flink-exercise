 ...
 */
public class OffsetKafka2HbaseAdapter {

    final static Logger logger = LoggerFactory.getLogger(OffsetKafka2HbaseAdapter.class);
    static Map  hbaseMap = new HashMap();
    static Properties p = new Properties();
    static {

        InputStream in = OffsetKafka2HbaseAdapter.class.getResourceAsStream("parameter.properties");
        try {
            p.load(in);
            //hbase 表信息
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            for (ConsumerRecord<String, String> record : records) {
                ...
                //从offset消费，解开注释: 更新kafka offset
                updateKafkaOffset(record.offset(),record.topic(),record.partition());
            }
        }
    }
...
//        }
     ...
     * @param kafkaConsumer
     */
    public static void setKafkaOffset(KafkaConsumer<String, String> kafkaConsumer){
        String[] topics = p.getProperty("kafka.topics").split(",");

        for (String topic:topics) {
            try {
                Map<String, Object> topicInfos = QueryKafkaTopicInfo.queryDataByName(topic);
                if(Objects.isNull(topicInfos)){
                    //第一次消费
                    System.out.println("第一次消费");
                    kafkaConsumer.subscribe(Arrays.asList(topics));
                }else {
                    System.out.println("不是第一次消费");
                    // 不是第一次
                    kafkaConsumer.assign(Arrays.asList(new TopicPartition(topic, (Integer) topicInfos.get("partitions"))));
                    //seekToBeginning 表示不改变当前的offset
                    //kafkaConsumer.seekToBeginning(Arrays.asList(new TopicPartition(topicName, 0)));
                    //seek表示指定特定的offset开始
                    kafkaConsumer.seek(new TopicPartition(topic, (Integer) topicInfos.get("partitions")), (Long) topicInfos.get("offset"));
                }
            } catch (SQLException e) {
                logger.info("获取offset失败......,error={}",e.getMessage());
            }
        }
     ...
     * @param
     */
    public static void updateKafkaOffset(long offset,String topicName,int partition){
        try {
            Map<String, Object> topicInfos = QueryKafkaTopicInfo.queryData(topicName,partition);
            if(Objects.isNull(topicInfos)){
                insertKafkaOffset(offset,topicName,partition);
            }else {
                QueryKafkaTopicInfo.updateData(topicName,offset,partition);
            }
        } catch (SQLException e) {
            logger.info("更新offset失败......,error={}",e.getMessage());
        }
    }
     ...
     * @param
     */
    public static void insertKafkaOffset(long offset,String topicName,int partition){
        try {
            QueryKafkaTopicInfo.insertData(topicName,offset,partition);
        } catch (SQLException e) {
            logger.info("插入offset失败......,error={}",e.getMessage());
        }
    }
